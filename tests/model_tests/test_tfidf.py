...
from src.models.tfidf import build_tfidf_matrix, get_similar_anime, recommend_tfidf


class TestBuildTfidfMatrix(unittest.TestCase):
    def setUp(self):
    ...
    def test_when_called_then_returns_dataframe(self):
        result = build_tfidf_matrix(self.anime_df)
    ...
    def test_when_called_then_columns_are_genre_words(self):
        result = build_tfidf_matrix(self.anime_df)
        self.assertIn("action", result.columns)
        self.assertIn("adventure", result.columns)

    def test_when_called_then_common_words_have_lower_weight_than_rare_words(self):
        # "action" appears in 2 anime, "romance" in 1 — romance should have higher IDF weight
        result = build_tfidf_matrix(self.anime_df)
        action_weight  = result.loc[1, "action"]
        romance_weight = result.loc[3, "romance"]
        self.assertGreater(romance_weight, action_weight)
...
class TestGetSimilarAnime(unittest.TestCase):
    def setUp(self):
        ...
        self.tfidf_matrix = build_tfidf_matrix(self.anime_df)

    def test_when_called_then_returns_dataframe(self):
        result = get_similar_anime(1, self.tfidf_matrix, self.anime_df)
        self.assertIsInstance(result, pd.DataFrame)
    ...
    def test_when_called_with_n_then_returns_n_results(self):
        result = get_similar_anime(1, self.tfidf_matrix, self.anime_df, n=2)
        self.assertEqual(len(result), 2)


class TestRecommendTfidf(unittest.TestCase):
    def setUp(self):
        self.anime_df = pd.DataFrame({
            "anime_id": [1,                    2,                  3,               4              ],
            "name":     ["A",                  "B",                "C",             "D"            ],
    ...
    def test_when_called_then_returns_dataframe(self):
        result = recommend_tfidf(1, self.train_df, self.tfidf_matrix, self.anime_df)
        self.assertIsInstance(result, pd.DataFrame)
    ...
    def test_when_called_with_n_then_returns_at_most_n_results(self):
        result = recommend_tfidf(1, self.train_df, self.tfidf_matrix, self.anime_df, n=2)
